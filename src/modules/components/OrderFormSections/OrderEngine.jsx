...
export default class OrderEngine {
    ...
    calculateTotalFreights = () => {
        ...
        for (let freight of this.context['freights']) {
            ...
            if (type === 'Skid') {
                this.context['total_pieces_skid'] += OrderEngine.calculateTotalPieces(normalpzedFreight, this.context['customer_weight_rules'])
            }
    ...
    static normalizeToElectronStandard = (freight) => {
        ...
        console.log('volnend: ', volumeWeight);
    ...
    format_value = () => {
        ...
        return ({
            total_pieces: Math.round(this.context['total_pieces'] * 100) / 100,
            total_actual_weight: Math.round(this.context['total_actual_weight'] * 100) / 100,
            total_pieces_skid: Math.round(this.context['total_pieces_skid'] * 100) / 100,
            total_volume_weight: Math.round(this.context['total_volume_weight'] * 100) / 100,
            total_weight_in_kg: Math.round(this.context['total_weight_in_kg'] * 100) / 100,
            total_chargeable_weight: Math.round(this.context['total_chargeable_weight'] * 100) / 100
        })
    }
